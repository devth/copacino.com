package com.cf.view.component.list.content
{
	...
	public class VideoControls extends com.cf.util.Component
	{
		...
		public function VideoControls( desiredWidth:Number, videoController:VideoController )
		{
			super();
			...
			//
			_videoController.addEventListener( VideoController.COMPLETE, videoController_event );
			_videoController.addEventListener( VideoController.METADATA, videoController_event );
			_videoController.addEventListener( VideoController.BUFFER_FULL, videoController_event );
			_videoController.addEventListener( VideoController.PLAY, videoController_event );
			...
			_volumeSlider = new VolumeSlider( volAreaWidth, HEIGHT,  PROGRESS_COLOR, GUTTER_COLOR, CONTROLS_COLOR, _videoController );
		...
		{
			if ( _videoController.status == VideoController.PLAYING )
			{
				_videoController.play( percent );
			}
			else
			{
				_videoController.play( percent );
		...
		{
			_buffer.scaleX = _videoController.getPercentLoaded();
			_progress.scaleX = _videoController.getPercentPlayed();
			
			isPlaying = ( _videoController.status == VideoController.PLAYING );
			...
			if ( _isSeeking )
			{
				...
				//MonsterDebugger.trace( this, "mouseX: " + _buffer.mouseX );
				//MonsterDebugger.trace( this, "lineWidth: " + _lineWidth );
...
//			stage.fullScreenSourceRect = rect;
		...
		{
			if ( _videoController.status != VideoController.PAUSED )
			{
				_videoController.pause();
				isPlaying = false;
			}
			...
			{
				dispatchEvent( new Event( VideoControls.PLAY_CLICKED ) );
		...
		private function videoController_event( e:Event ):void
		{
			...
			switch ( e.type )
			{
				case VideoController.COMPLETE:
				
				break;
				case VideoController.BUFFER_FULL:
				
				break;
				case VideoController.PLAY:
				
				break;
				case VideoController.METADATA:
